...
// ===================== LHS-1E parameters =====================
// Table is in g/cm^3 → multiply by 1000 to get kg/m^3
constexpr float LHS1E_BULK_LOOSE = 1.27f * 1000.0f;   // 1270 kg/m^3
constexpr float LHS1E_BULK_TARGET = 1.70f * 1000.0f;  // 1700 kg/m^3
constexpr float LHS1E_BULK_MAX = 1.86f * 1000.0f;     // 1860 kg/m^3

// Grain (solid) density for basaltic simulant (typical 2.9–3.1 g/cm^3)
constexpr float LHS1E_PARTICLE_RHO = 3.00f * 1000.0f;  // 3000 kg/m^3

// Friction / restitution tuned more like regolith (less bouncy)
constexpr float LHS1E_mu = 0.65f;   // tan(phi) ~ 0.65 (phi ≈ 33–40°)
constexpr float LHS1E_CoR = 0.30f;  // more dissipative than 0.8
// =============================================================
...
int main() {
    ...
    // NEW (LHS-1E) materials
    auto mat_type_cone = DEMSim.LoadMaterial({{"E", 1e9}, {"nu", 0.3}, {"CoR", 0.40}, {"mu", 0.70}, {"Crr", 0.00}});
    auto mat_type_terrain =
        DEMSim.LoadMaterial({{"E", 1e9}, {"nu", 0.3}, {"CoR", LHS1E_CoR}, {"mu", LHS1E_mu}, {"Crr", 0.00}});

    // Pair-specific overrides for cone–soil contacts
    DEMSim.SetMaterialPropertyPair("CoR", mat_type_cone, mat_type_terrain, 0.40f);
    DEMSim.SetMaterialPropertyPair("mu", mat_type_cone, mat_type_terrain, std::max(0.70f, LHS1E_mu));
    ...
    double scale = 0.0028;   // NEW (LHS-1E) finer → more particles
